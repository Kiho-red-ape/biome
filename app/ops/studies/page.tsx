    ...
    <div>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-mono)', fontSize: 11 }}>
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={9} style={{ padding: 24, color: '#4a6050', textAlign: 'center' }}>No studies yet.</td></tr>
            )}
            {rows.map((s) => (
              <tr key={s.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                <td style={{ padding: '10px 12px', color: '#4a6050' }}>{(s.experiment_code as string | null) ?? '—'}</td>
                ...
                <td style={{ padding: '10px 12px' }}>
                  <span style={{ color: STATUS_COLOR[s.status as string] ?? '#4a6050' }}>{s.status as string}</span>
                </td>
                ...
                <td style={{ padding: '10px 12px', color: s.is_verified ? '#b7ff61' : '#4a6050' }}>{s.is_verified ? '✓' : '—'}</td>
                <td style={{ padding: '10px 12px', color: '#4a6050', whiteSpace: 'nowrap' }}>
