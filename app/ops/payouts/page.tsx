    ...
    <div>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-mono)', fontSize: 11 }}>
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={8} style={{ padding: 24, color: '#5b8a9a', textAlign: 'center' }}>No payouts with status: {ps}</td></tr>
            )}
            {rows.map((p) => {
              const exp = p.experiments as { title: string } | null;
              return (
                <tr key={p.id as string} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  <td style={{ padding: '8px 12px', color: '#5b8a9a', fontSize: 10 }}>{(p.id as string).slice(0, 8)}…</td>
                  ...
                  <td style={{ padding: '8px 12px', color: '#5b8a9a' }}>
                  ...
                  <td style={{ padding: '8px 12px', color: '#5b8a9a', fontSize: 10 }}>
