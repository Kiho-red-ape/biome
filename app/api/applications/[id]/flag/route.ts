...
// Experimenter flags a participant for a compliance violation
export async function PATCH(req: NextRequest, { params }: Props) {
  ...
  // Validate experimenter owns the experiment this application belongs to
  ...
  const { data: updated, error } = await supabase
    .from('applications')
    .update({ violation_flagged: true, violation_reason: reason.trim() })
    .eq('id', id)
    .select()
    .single();
