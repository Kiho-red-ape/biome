...
export default function ParticipantAgreementPage() {
  ...
  async function handleAccept() {
    ...
    await fetch('/api/legal/accept', {
      ...
      body:    JSON.stringify({ privy_did: user.id, doc_key: 'participant_agreement' }),
    });
    setLoading(false);
    setAccepted(true);
  }
      ...
      <div className="flex-1 flex items-start justify-center px-4 py-8">
        {accepted ? (
          <div className="text-center py-16">
            ...
            <p className="mono text-sm" style={{ color: 'var(--text-bright)' }}>Participant Study Agreement accepted.</p>
          </div>
        ) : (
          <LegalAcceptance
            docKey="participant_agreement"
            title={doc.title}
            content={doc.content}
